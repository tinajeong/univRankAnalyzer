...
위의 내용을 다 이해했다면 다음과 같은 활용도 가능합니다. 
...
China를 tracking 할 수 있는 정보는 t-taut class를 통해 추출해야 합니다.
...
3. ```java
   ...
   위 코드는 다른 페이지에서 대학순위를 보여줄때 다른 DOM 구조를 가질때도 있어서 general한 적용을 위해 수정한 코드입니다. text() 함수는 ```<span class="rankscore-bronze">#1</span>``` 태그 정보를 제외한 #1같은 텍스트 정보만 가져오는 함수입니다.
...
5. ```java
   ...
   t-taut class는 나라와 도시 정보가 함께 나와있었으므로 그 중 첫번쨰 원소인 나라 이름만 가져옵니다.
